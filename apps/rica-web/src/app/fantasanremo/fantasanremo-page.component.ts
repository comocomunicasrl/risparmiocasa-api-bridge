...
export class FantasanremoPageComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    readonly OPENING_DATE = new Date("2025-01-23T12:00:00.000+01:00").getTime();
    ...
    ngOnInit() {
        ...
        if (!this.route.snapshot.queryParamMap.has('bypass') && (isPlatformBrowser(this.platformId)) && ((this.ACTUAL_DATE < this.OPENING_DATE) || (this.ACTUAL_DATE > this.CLOSING_DATE))) {
    ...
    ngAfterViewInit(): void {
        if (isPlatformBrowser(this.platformId) && this.cardNumberEl?.nativeElement) {
